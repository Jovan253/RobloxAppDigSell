...
- [x] Reel used to land exactly on the last item (dead stop, no wheel illusion). Fixed: result now lands at index 42 of 50, leaving 8 decorative items visible sliding past after it settles.
- [x] "My Skins" was revealing the won skin the instant the server granted it (PlayerData.OwnedSkins replicates immediately, well before the 7s reel finishes), spoiling the reveal. Fixed: a `spinning` flag now gates the auto-refresh on ownership/equip changes while the reel is running, and the inventory is refreshed manually once the spin's tween actually completes.
- [x] Confirmed working live — Gem system Stage 2 (Speed Boost + case-opening skins) is fully done, closing out the Gem system's whole v1 scope (Collection Journal + Speed Boost + cases)
- Future idea (not built): show an actual rendered image of the pickaxe with the skin applied (e.g. a ViewportFrame) instead of a flat color swatch in the reel/inventory rows. User: "would be nicer... but this is ok" -- explicitly a nice-to-have, not blocking.
